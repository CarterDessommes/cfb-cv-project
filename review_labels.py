...
LABELS_CSV   = Path(__file__).parent / "game_db" / "labels.csv"
DISPLAY_SIZE = 640
...
class Reviewer:
    def __init__(self, root: tk.Tk):
        ...
        self.df = pd.read_csv(LABELS_CSV, dtype={"label": str})
        self._orig_labels = self.df["label"].copy()
        self._undo_stack: list[tuple[int, str]] = []
    ...
    def _apply_filter(self):
        ...
        if fv == "All":
            self._filtered_indices = list(range(len(self.df)))[::-1]
        else:
    ...
    def _save(self):
        self.df.to_csv(LABELS_CSV, index=False)
