"""
Extract jersey crops from a video and optionally label them with a VLM.

Two modes:
  --extract-only   Run detection only. Save crops to out-dir/crops/, write
                   placeholder labels. Use this to check crop quality first.
...
Usage:
    python3 label_crops.py <video> --out-dir <dir>
                           ...
                           [--extract-only]
                           [--max N]           stop after N crops
                           [--max-frames N]    stop at frame number N
                           [--start-frame N]   seek to frame N before extracting
                           [--vlm-model ID]    default: Qwen/Qwen2.5-VL-3B-Instruct
...
def run(video_path, out_dir, det_path, det_conf, every_n, extract_only,
        max_crops, max_frames=None, start_frame=0,
    ...
    with open(labels_path, "a" if append else "w", newline="") as csv_f:
        ...
        while cap.isOpened():
            ...
            for box in boxes:
                ...
                if blur < _EXTRACT_BLUR_MIN:
                    continue
                if crop.shape[0] < _EXTRACT_MIN_H or crop.shape[1] < _EXTRACT_MIN_W:
                    continue
...
if __name__ == "__main__":
    ...
    run(video, out_dir, det_path, det_conf, every_n, extract_only,
